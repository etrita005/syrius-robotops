...
const REMOTE_TARGET_PATH = "/tmp/ggr3_config.zip";

export class TransferGGR3ConfigTask extends SshFileTransferTask {
  ...
  protected override async onExec(params: ValueMap, context?: ValueMap): Promise<ValueMap> {
    ...
    if (artifactId && artifactService) {
      ...
      this.log.info({ artifactId, localFilePath }, 'Resolved GGR3 config artifact path');
