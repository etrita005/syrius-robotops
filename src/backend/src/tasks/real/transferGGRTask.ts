...
const REMOTE_TARGET_PATH = "/home/developer/ggr_package.apk";

export class TransferGGRTask extends SshFileTransferTask {
  ...
  protected override async onExec(params: ValueMap, context?: ValueMap): Promise<ValueMap> {
    ...
    if (artifactId && artifactService) {
      const tmpDir = join(tmpdir(), `ggr-transfer-${Date.now()}`);
