...
describe("Deploy AE Config tasks", () => {
  ...
  it("TC-AE-001: DeployAEConfigTask builds the multi-step deploy command with sudo", () => {
    ...
    const fragments = [
      ...
      "rm -rf /tmp/ae_config_extract/*",
      "unzip -o /tmp/ae_config_package.zip -d /tmp/ae_config_extract",
      // Copy the inner wrapper's contents (via the */. glob), not the wrapper
      // directory itself, so /opt/cosmos/bin/applet-engine holds inner files
      // directly.
    ...
    // Negative assertion: must NOT copy the extract dir directly (which would
    // include the wrapper folder), only the inner directory's contents.
    assert.equal(
      cmd.includes("cp -rf /tmp/ae_config_extract/. /opt/cosmos/bin/applet-engine/"),
      false,
      "Must not copy the extract dir as a whole; only the inner wrapper's contents"
    );

    // Negative assertion: must NOT use /home/developer for the upload/extract
    // staging area (the package and extract dir live under /tmp).
    assert.equal(
