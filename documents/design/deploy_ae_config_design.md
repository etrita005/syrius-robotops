...
│   (deploy-ae-config DAG) │    │ - ResolverRegistry     │    │      ae_config_extract/     │
...
| `REMOTE_EXTRACT_DIR`  | `/tmp/ae_config_extract`     | 解压临时目录 |
| `REMOTE_DEPLOY_DIR`   | `/opt/cosmos/bin/applet-engine`         | 最终部署目录 |
...
- 复制步骤使用 glob 模式 `${REMOTE_EXTRACT_DIR}/*/.`：`*/` 匹配解压目录下的所有顶层子目录（即 AE 压缩包内层包装目录），尾部 `/.` 表示这些目录的**内容**而非目录本身。结果是 `/opt/cosmos/bin/applet-engine/` 中获得 AE 压缩包真正的有效文件，而不是嵌套一层外层包装目录。
