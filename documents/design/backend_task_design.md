...
- Does not clean up the `.deb` file itself; cleanup happens via the errorDag (`SshCommandTask` running `rm -f /tmp/dragonball3_package.deb`) when installation fails. On success, the subsequent `RebootRobotTask` discards the file via reboot.
