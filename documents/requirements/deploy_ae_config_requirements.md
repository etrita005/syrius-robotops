...
| **解压临时目录** | 不再使用解压临时目录；压缩包直接通过 `unzip -o` 解压到 `/opt/cosmos/bin/applet-engine/` 部署目录。 |
...
2. **直接解压**：通过 `unzip -o` 将压缩包直接解压到 `/opt/cosmos/bin/applet-engine/`，不再创建/使用 `/tmp/ae_config_extract` 中转目录；发生异常时通过 errorDag 清理 `/tmp/ae_config_package.zip`。
3. **最小破坏性**：默认仅以 `unzip -o` 覆盖目标目录中的同名文件，不会清空目标目录中未在压缩包内的文件，避免误删除现场已有依赖。
...
1. `transfer`：调用 `TransferAEConfigTask`，将制品通过 SFTP 推送到机器人 `/tmp/ae_config_package.zip`。
2. `deploy`：调用 `DeployAEConfigTask`，在机器人侧执行：
   - 检查 `/opt/cosmos/bin/applet-engine` 目录是否存在；**不存在则直接以非零退出码失败**（不会自动创建该目录）。
   - `unzip -o /tmp/ae_config_package.zip -d /opt/cosmos/bin/applet-engine` —— 直接将压缩包内容解压到部署目录，不再使用 `/tmp/ae_config_extract` 中转目录。
...
| AC-AE-004 | 任务成功后，机器人 `/tmp/ae_config_package.zip` 不再存在。 |
