...
**前置条件**：使用 FailingMockInstall 注册，并注册 `MockDeleteDragonball3Task`

**测试步骤**：
1. 创建 `FailingMockInstallDragonball3Task`（onExec 抛出错误）
2. 注册到 engine，errorDag 使用 `DeleteDragonball3Task`
3. 创建 flow，等待 flow 完成

**预期结果**：
- Flow 达到 `FAILED` 状态
- `phase` 为 `error`
- `error_cleanup` 任务被执行（`DeleteDragonball3Task`）

### TC-DB3-010: DeleteDragonball3 生成正确的清理命令

**前置条件**：无

**测试步骤**：
1. 创建 `TestableDeleteDragonball3Task` 实例
2. 调用 `getSshCommand()`

**预期结果**：
- 命令内容为 `rm -f /tmp/dragonball3_package.deb`

### TC-DB3-011: DeleteDragonball3 使用 sudo
