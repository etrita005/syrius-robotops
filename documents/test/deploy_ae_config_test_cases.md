...
| 预期 | `sshCommand` 包含按顺序出现的关键片段：`[ -d /opt/cosmos/bin/applet-engine ] || { echo "Deploy target not found: /opt/cosmos/bin/applet-engine" >&2; exit 1; }`、`unzip -o /tmp/ae_config_package.zip -d /opt/cosmos/bin/applet-engine`、`chown -R cosmos:cosmos /opt/cosmos/bin/applet-engine`、`rm -f /tmp/ae_config_package.zip`；命令字符串中**不**包含 `mkdir -p /opt/cosmos/bin/applet-engine`（不自动创建部署目录），**也不**包含 `/tmp/ae_config_extract`（不再使用中转解压目录），**也不**包含 `/home/developer`（暂存路径必须位于 `/tmp/`）；`buildParams` 返回的 `sudo === true`，`commandTimeout === 60000`。 |
...
| AC-AE-004 | TC-AE-001（rm -f 末尾段）+ TC-AE-002 |
