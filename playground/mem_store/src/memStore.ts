...
const cache = new LRUCache<string, CacheValuePayload>({
  ...
  dispose: (value, key, reason) => {
    if (reason === 'expire') {
      ...
      console.log(`[LRU] Expired and fully cleaned key: ${key}`);
    } else if (reason === 'delete') {
      console.log(`[LRU] Deleted key: ${key}`);
    } else if (reason === 'evict') {
      console.log(`[LRU] Evicted value for key: ${key}, keeping meta and jobs`);
    } else if (reason === 'set') {
      clearJobsForKey(key as string);
    }
  },
...
export function getCache(key: string): unknown | undefined {
  ...
  if (meta) {
    triggerRefresh(key).catch((err: Error) => console.error(`[MissRefresh] ${key}:`, err.message));
  }
  return undefined;
}
...
export function getMeta(key: string): { dag: Dag; config: CacheConfig; payload?: CacheValuePayload } | undefined {
  const meta = metaStore.get(key);
...
export async function triggerRefresh(key: string): Promise<unknown> {
  ...
  const promise = (async () => {
    try {
      console.log(`[Refresh] Starting refresh for key: ${key}`);
      const result = await executeDag(meta.dag);
      updateCache(key, result);
      console.log(`[Refresh] Completed refresh for key: ${key}`);
      return result;
    } catch (err) {
      console.error(`[Refresh] Failed for key ${key}:`, err);
      throw err;
    } finally {
...
function setupSchedule(key: string, config: CacheConfig, expireAt: number) {
  if (config.preExpireWarningMs && config.preExpireWarningMs > 0) {
    ...
    scheduleWarning(key, warningDelay, async () => {
      console.log(`[Warning] Pre-expire warning for key: ${key}`);
      await triggerRefresh(key);
    });
  }
  if (config.cron) {
    scheduleCron(key, config.cron, async () => {
      console.log(`[Cron] Periodic refresh for key: ${key}`);
      await triggerRefresh(key);
    });
  }
}
