...
export function scheduleCron(key: string, cron: string, callback: () => Promise<void>) {
  ...
  const task = new AsyncTask(
    id,
    callback,
    (err: Error) => console.error(`Cron task error for ${key}:`, err.message)
  );
...
export function scheduleWarning(key: string, delayMs: number, callback: () => Promise<void>) {
  ...
  if (delayMs <= 0) {
    callback().catch((err: Error) => console.error(`Warning immediate error for ${key}:`, err.message));
    return;
  }
  const timeout = setTimeout(() => {
    timeoutMap.delete(id);
    callback().catch((err: Error) => console.error(`Warning task error for ${key}:`, err.message));
  }, delayMs);
...
function parseCronToSeconds(cron: string): number {
  ...
  console.warn(`Unsupported cron format "${cron}", defaulting to 10s interval`);
  return 10;
}
